  ...
  return isLoading ? (
    ...
    <section className='p-4 lg:p-20'>
      <div className='flex flex-row justify-between py-5'>
        ...
        <Link href={'/todas_noticias'} className='font-bold hover:text-[#275DAA] text-sm md:text-lg'>
          MOSTRAR TODAS
        </Link>
      </div>
      ...
      <section className='grid grid-cols-4 gap-x-4 gap-y-6 flow-row-dense'>
        {noticias
          ...
          .map((noticia, index) =>
            index == 0 ? (
              <article key={index} className='col-span-4 md:col-span-3 cursor-pointer'>
                <Link href='noticia' key={index} onClick={() => setNoticiaActive(noticia)}>
                ...
                md:space-y-5'
                  ...
                  uppercase '
                          ...
                          <h1 className='text-4xl font-bold leading-none lg:text-5xl xl:text-6xl'>{noticia.titulo}</h1>
                          <div className='pt-2 pr-0 pb-0 pl-0'>
                            <p className='flex items-center text-sm font-medium mt-0 mr-1 mb-0 ml-1'>
                              <FaCalendar className='mr-2 text-lg text-[#275DAA]' /> {dayjs(noticia.updatedAt).format('LL')}
                            </p>
                          </div>
                        </div>
                      </div>
                      <div>
                        <p className='py-4 items-self-left'>{noticia.descripcion_corta}</p>
                      </div>
