    ...
    <footer className='bg-[#14233d]'>
      <div className='mx-auto max-w-screen-xl space-y-2 px-4 py-16 sm:px-6 lg:space-y-2 lg:px-8'>
        <div className='sm:flex sm:items-center sm:justify-between'>
          ...
          {/* START ICON SOCIALMEDIA */}
          <ul className='mt-8 flex justify-start gap-6 sm:mt-0 sm:justify-end'>
            <li>
              <a href='/' rel='noreferrer' target='_blank' className='text-[#c6d7f1] transition hover:opacity-75'>
            ...
            <div key={index} className='text-center'>
              ...
              <ul className='mt-6 space-y-4 text-sm'>
                {childOptions
                  ...
                  .map((route, index) => (
                    <li key={index}>
                      <Link href={route.path} className='text-[#c6d7f1] transition hover:opacity-75'>
                        {route.name}
                      </Link>
                    </li>
                  ))}
        ...
        <p className='text-xs text-[#f3f6fc] text-center'>&copy; 2023. AMFRED A.C. Todos los derechos reservados.</p>
      </div>
    </footer>
  )
}
